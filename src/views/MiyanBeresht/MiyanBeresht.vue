...
// overlay-based transform animation to avoid layout reflow on detach/attach
async function runAttachSequenceBeresht() {
...
async function runDetachSequenceBeresht() {
  if (!navbarRef.value || !overlayRef.value) return
  const r = navbarRef.value.getBoundingClientRect()
  overlayRef.value.innerHTML = navbarRef.value.innerHTML
...
watch(attached, (newVal, oldVal) => {
  if (oldVal === false && newVal === true) runAttachSequenceBeresht()
  if (oldVal === true && newVal === false) runDetachSequenceBeresht()
}, { immediate: true })
